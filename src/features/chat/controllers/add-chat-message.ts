...
export class Add {
  ...
  public async message(req: Request, res: Response): Promise<void> {
    ...
    if (selectedImage.length) {
      const result: UploadApiResponse = (await uploads(req.body.selectedImage, req.currentUser!.userId, true, true)) as UploadApiResponse;
      if (!result?.public_id) {
        throw new BadRequestError(result.message);
      }
      fileUrl = `https://res.cloudinary.com/df1ejejij/image/upload/v${result.version}/${result.public_id}`;
    }
